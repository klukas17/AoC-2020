def task():
    ...
    for instruction in instructions:
        ...
        if instruction == "start" or instruction == "end" or instruction[0][0] == "acc":
            continue
        
        formerInstruction = instruction[0]

        if formerInstruction == "jmp":
            instruction[0] = "nop"

        elif formerInstruction == "nop":
            instruction[0] == "jmp"

        result = execute(instructions)

        if result == False:
            instruction[0] = formerInstruction
